...
(function () {
  ...
  var config = {
    ...
    "defaultTheme": "kobo",
    "languagesSupported": ["ar", "cs", "de", "el", "en", "es", "fa", "fi", "fr", "hi", "it", "ka", "lo", "nl", "no", "pl", "pt", "ro", "ru", "sk", "sq", "sv", "sw", "tr", "vi", "zh"],
    "timeout": 300000,
    "submissionParameter": {
      "name": ""
    },
  ...
  const dataUriCache = {}; //var hasArrayBufferView = new Blob( [ new Uint8Array( 100 ) ] ).size == 100;
  ...
  function csvToArray(csv) {
    ...
    const result = papaparse_min.parse(csv.trim(), options);
  ...
  function arrayToXml(rows, langMap) {
    //var xmlStr;
    let headers = rows.shift(); //var langAttrs = [];

    const langs = [];
    langMap = typeof langMap !== 'object' ? {} : langMap; // Trim the headings

    headers = headers.map(header => header.trim()); // Extract and strip languages from headers
    ...
    headers = headers.map((header, index) => {
      ...
      if (parts && parts.length === 2) {
        lang = langMap[parts[1]] || parts[1]; //langAttrs[ index ] = ' lang="' + lang + '"';
    ...
    headers.every(_throwInvalidXmlNodeName); // create an XML Document
  ...
  function _serializeQueryComponent(name, value) {
    let n;
    let serialized = ''; // for both arrays of single-level objects and regular single-level objects
  ...
  function getEnketoId(path) {
    ...
    const id = path.substring(path.lastIndexOf('/') + 1); // previews /preview, and /preview/i can be loaded (unofficially) without an ID.
  ...
  const settingsMap = [{
    ...
    q: 'instance_id',
    s: 'instanceId'
  }, 'parentWindowOrigin', 'print', 'format', 'landscape', 'margin', 'touch']; // rename query string parameters to settings, but only if they do not exist already
  ...
  settings.loginUrl = config['basePath'] + DEFAULT_LOGIN_URL; // add default return Url

  settings.defaultReturnUrl = config['basePath'] + DEFAULT_THANKS_URL; // add defaults object
  ...
  } : undefined; // set default maxSubmissionSize

  settings.maxSize = DEFAULT_MAX_SIZE; // add type
  ...
  settings.offlinePath = settings.offline ? '/x' : ''; // Extract Enketo ID

  settings.enketoId = utils.getEnketoId(window.location.pathname); // Set multipleAllowed for single webform views
  ...
  settings.goTo = settings.type === 'edit' || settings.type === 'preview' || settings.type === 'view'; // A bit crude and hackable by users, but this way also type=view with a record will be caught.
